...
[condition][]who has donated {number} gifts=$gifts : ArrayList(size=={number}) from collect ($gift : Gift(person.id == $personId))
...
[consequence][]Ensure that the person is not a major donor=$person.setMajorDonor(false);
