...
UPDATE PERSON SET MAJOR_DONOR = TRUE WHERE PERSON_ID IN (SELECT PERSON_ID FROM GIFT WHERE AMOUNT > 0 AND (TRANSACTION_DATE BETWEEN (DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -1 YEAR)) AND CURRENT_TIMESTAMP) GROUP BY PERSON_ID HAVING (SUM(AMOUNT)>=5000));
