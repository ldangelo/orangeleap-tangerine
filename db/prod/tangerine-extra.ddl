...
# Dump of table BATCH_REVIEW_SET_ITEM
# ------------------------------------------------------------

DROP TABLE IF EXISTS `BATCH_REVIEW_SET_ITEM`;

CREATE TABLE `BATCH_REVIEW_SET_ITEM` (
  `BATCH_REVIEW_SET_ITEM_ID` bigint(20) NOT NULL auto_increment,
  `BATCH_ID` bigint(20) NOT NULL,
  `ENTITY_ID` bigint(20) NOT NULL,
  `CREATE_DATE` datetime default NULL,
  `UPDATE_DATE` datetime default NULL,
  PRIMARY KEY  (`BATCH_REVIEW_SET_ITEM_ID`),
  CONSTRAINT `FK_BATCH_REVIEW_SET_ITEM_BATCH_ID` FOREIGN KEY (`BATCH_ID`) REFERENCES `BATCH` (`BATCH_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
