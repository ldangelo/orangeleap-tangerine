...
INDEX `CUSTOM_FIELD_ENTITY_IDX` (`ENTITY_ID`,`ENTITY_TYPE`,`FIELD_NAME`),
INDEX `IDX_CUSTOM_FIELD_FIELD_VALUE` (`ENTITY_TYPE`,`FIELD_NAME`,`FIELD_VALUE`(10))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
KEY `IDX_CONSTITUENT_ORGANIZATION_NAME` (`ORGANIZATION_NAME`),
KEY `IDX_CONSTITUENT_UPDATE_DATE` (`UPDATE_DATE`),
...
KEY `KEY_ADDRESS_POSTAL_CODE` (`POSTAL_CODE`,`CONSTITUENT_ID`),
...
`CHECK_HOLDER_NAME` varchar(255) default NULL,
`CHECK_ACCOUNT_NUMBER` varchar(255) DEFAULT NULL,
`CHECK_ROUTING_NUMBER` varchar(255) DEFAULT NULL,
`INACTIVE` char(1) NOT NULL default '0',
...
`SITE_NAME` varchar(255) default NULL,
`DISPLAY_COLLAPSED` char(1) NOT NULL default '0',
...
KEY `IDX_COMMUNICATION_HISTORY_BATCH_ID` (`BATCH_ID`),
KEY `IDX_COMMUNICATION_ADJ_GIFT_ID` (`ADJUSTED_GIFT_ID`),
...
`POST_BATCH_DESC` varchar(255) NULL,
`ENTITY` varchar(255) NOT NULL,
`BATCH_CREATED_BY_ID` bigint(20) default NULL,
`BATCH_CREATED_DATE` datetime default NULL,
`EXECUTED` char(1) NOT NULL default '0',
`EXECUTED_BY_ID` bigint(20) default NULL,
...
CONSTRAINT `FK_POST_BATCH_CREATED_BY_ID` FOREIGN KEY (`BATCH_CREATED_BY_ID`) REFERENCES `CONSTITUENT` (`CONSTITUENT_ID`),
CONSTRAINT `FK_POST_BATCH_POSTED_BY_ID` FOREIGN KEY (`POSTED_BY_ID`) REFERENCES `CONSTITUENT` (`CONSTITUENT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

# Dump of table POST_BATCH_ENTRY
# ------------------------------------------------------------
...
# Dump of table POST_BATCH_ENTRY_ERROR
# ------------------------------------------------------------
...
INDEX `THEGURU_SEGMENTATION_RESULT_REPORT_ID_IDX` (`REPORT_ID`,`ENTITY_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
DROP VIEW IF EXISTS `V_ROLLUP_VALUE`;
...
-- DSL (semi-static, not usually site specific)
...
DELETE FROM VERSION;
INSERT INTO VERSION (COMPONENT_ID, COMPONENT_DESC, SCHEMA_MAJOR_VERSION, SCHEMA_MINOR_VERSION) VALUES ('ORANGE', 'Orange Leap', 8, 0);
