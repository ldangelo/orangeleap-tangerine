...
@Table(name = "SECTION_FIELD", uniqueConstraints = @UniqueConstraint(columnNames = {
		"SECTION_NAME", "FIELD_DEFINITION_ID", "SECONDARY_FIELD_DEFINITION_ID",
		"SITE_ID" }))
public class SectionField implements Serializable {
	...
	public void setSecondaryFieldDefinition(
			FieldDefinition secondaryFieldDefinition) {
	...
	public String getFieldRequiredName() {
		if (!isCompoundField()) {
			...
			return fieldDefinition.getEntityType().name() + "."
					+ fieldDefinition.getFieldInfo() + "."
					+ secondaryFieldDefinition.getFieldName();
		}
	}
