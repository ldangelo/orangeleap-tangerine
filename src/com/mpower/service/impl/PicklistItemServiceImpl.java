...
public class PicklistItemServiceImpl implements PicklistItemService {
    ...
    private PicklistDao picklistItemDao;
    ...
    private boolean exclude(Picklist picklist) {
        ...
        return name == null || name.equals("frequency") || name.equals("constituentType") || name.equals("phoneType") // editing the default values of these person info items may cause problems, so disable editing for now
    ...
    public Picklist getPicklist(String picklistId) {
        ...
        if (picklist == null) {
            picklist = picklistItemDao.readPicklistById(removeSiteFromId(picklistId)); // TODO: is the removeSiteFromId invocation necessary?
        }
        if (picklist == null) {
            return null;
        }
        ...
        if (picklist.getSite() == null) {
            return createCopy(picklist);
        } 
//        else if (picklist.getSite().getName().equals(siteName)) { TODO: is this check necessary?
//            return picklist;
//        } 
        else {
            return null;
    ...
    public List<Picklist> listPicklists() {
        List<Picklist> list = picklistItemDao.listPicklists();
    ...
    private Picklist createCopy(Picklist template) {
        ...
        try {
            ...
            result.setId(addSiteToId(template.getSite().getName(), template.getId()));
            result.setSite(template.getSite());//getSite(siteName));
...
//    }
    ...
    public PicklistItem maintainPicklistItem(PicklistItem picklistItem) {
        ...
        if (picklist == null || !picklist.getId().contains("-") || picklist.getSite() == null || picklist.getSite().getName().length() == 0) {
        ...
        if (dbPicklist == null) {
            ...
            if (picklistItem.getId() != null) {
                ...
                try {
                    ...
                    //picklistItem.setOriginalObject(oldPicklistItem); TODO: put back for IBatis
