...
public class AdjustedGiftController extends TangerineConstituentAttributesFormController {
    ...
    protected ModelAndView showForm(HttpServletRequest request, HttpServletResponse response, BindException errors) throws Exception {
        ...
        if (giftControllerHelper.showAdjustedGiftPaidView(adjustedGift)) {
            redirectUrl = giftControllerHelper.appendAdjustedGiftParameters(giftControllerHelper.getAdjustedGiftPaidUrl(), adjustedGift, getConstituentId(request));
        }
        else if (giftControllerHelper.showAdjustedGiftPostedView(adjustedGift)) {
            redirectUrl = giftControllerHelper.appendAdjustedGiftParameters(giftControllerHelper.getAdjustedGiftPostedUrl(), adjustedGift, getConstituentId(request));
        }
        if (redirectUrl != null) {
    ...
    protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException formErrors) throws Exception {
	    ...
	    try {
            ...
            if (giftControllerHelper.showAdjustedGiftPaidView(anAdjustedGift)) {
                String redirectUrl = giftControllerHelper.appendAdjustedGiftParameters(giftControllerHelper.getAdjustedGiftPaidUrl(), anAdjustedGift, getConstituentId(request));
                mav = new ModelAndView(redirectUrl);
            }
            else if (giftControllerHelper.showAdjustedGiftPostedView(anAdjustedGift)) {
