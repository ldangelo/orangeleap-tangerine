        ...
        synchronized(JobSchemaIterator.class) {
            ...
            for (String schema : schemas) {
            	try {
	                ...
	                logger.info("Processing jobs for [" + schema + "] on " + new java.util.Date() + " ---------------------------------------------------");  // Must log after changing schema context
	                long t = System.currentTimeMillis();
	                executeInternalForSchema(context, applicationContext);
	                logger.info("Jobs for schema " + schema + " took " + (System.currentTimeMillis() - t)/1000.0f/60f + " minutes.  Complete on " + new java.util.Date());
            	} catch (Exception e) {
            		e.printStackTrace();
            		logger.error("Error processing job for " + schema + " on " + new java.util.Date(), e);
            	}
            }
