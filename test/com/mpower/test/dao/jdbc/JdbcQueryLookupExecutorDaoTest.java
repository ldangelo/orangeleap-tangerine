...
public class JdbcQueryLookupExecutorDaoTest extends AbstractTestNGSpringContextTests {
    ...
    public void testExecuteQueryLookupNoResults() throws Exception {
        ...
        List<Object> results = queryLookupExecutorDAO.executeQueryLookup("SELECT constituent_id, organization_name, last_name, first_name, middle_name, suffix FROM CONSTITUENT WHERE site_name = :siteName AND constituent_type = 'individual' AND title = 'Sucka.'", parameters);
    ...
    public void testExecuteQueryLookup() throws Exception {
        Map<String, String> parameters = new LinkedHashMap<String, String>(1); 
        parameters.put("siteName", "company1");
        List<Object> results = queryLookupExecutorDAO.executeQueryLookup("SELECT constituent_id, organization_name, last_name, first_name, middle_name, suffix FROM CONSTITUENT WHERE site_name = :siteName AND constituent_type = 'individual' AND title = 'Rev.'", parameters);
