...
<page:applyDecorator name="form">
	<spring:message code='editAddress' var="titleText" scope="request" />
	...
	<html>
		...
		<body>
			<form:form method="post" commandName="${requestScope.commandObject}">
				...
				<tangerine:fields pageName="addressManagerEdit"/>
				...
				<div class="formButtonFooter constituentFormButtons">
					...
					<c:if test="${pageAccess['/addressManager.htm']!='DENIED'}"> <%-- TODO: --%>
                        <input type="button" value="<spring:message code='cancel'/>" class="saveButton" onclick="OrangeLeap.gotoUrl('addressList.htm?constituentId=${constituent.id}')"/>
					</c:if>
				</div>
			</form:form>
		</body>
    ...
    <page:param name="scripts">
        <script type="text/javascript">
            var ButtonPanel = Ext.extend(Ext.Panel, {
                ...
                menu : {
                    items: [
                        { text: '<spring:message code='enterNew'/>', handler: function() { OrangeLeap.gotoUrl("addressManager.htm?constituentId=${requestScope.constituent.id}"); } }
                    ]
                },
                split: false,
