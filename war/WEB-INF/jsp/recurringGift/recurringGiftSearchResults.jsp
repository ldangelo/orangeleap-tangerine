...
<c:when test="${!empty recurringGiftList}">
	<div class="searchResultsHeader">
		<div class="pagination"><span class="disabled">« Previous</span> <span class="current">1</span> <a href="recurringGiftSearchResults.jsp#">2</a> <a href="recurringGiftSearchResults.jsp#">3</a> <a href="recurringGiftSearchResults.jsp#">Next »</a></div>
				...
				<c:forEach items="${recurringGiftList}" var="row">
					<c:choose>
						<c:when test="${empty row.gifts}">
							<tr>
								<td><a href="recurringGift.htm?recurringGiftId=${row.id}&constituentId=${row.constituent.id}">View</a></td>
								<%@ include file="/WEB-INF/jsp/snippets/gridResults.jsp" %>
							</tr>
						</c:when>
						<c:otherwise>
							<tr>
								<td><a href="recurringGiftView.htm?recurringGiftId=${row.id}&constituentId=${row.constituent.id}">View</a></td>
								<%@ include file="/WEB-INF/jsp/snippets/gridResults.jsp" %>
							</tr>
						</c:otherwise>
					</c:choose>
