...
<c:if test="${row != null}">
	<tbody class="<c:if test='${not empty hiddenGridRows}'>expandable</c:if><c:choose><c:when test='${counter == 0}'> noDisplay</c:when><c:otherwise> gridRow</c:otherwise></c:choose>" 
		...
		<tr class="lineRow <c:if test='${not empty hiddenGridRows}'>expandableRow collapsed</c:if>">
			...
			<td><a href="#" class="<c:if test='${counter == 0 || (counter == 1 && fn:length(gridCollection) == 1)}'>noDisplay</c:if> deleteButton"><img src="images/icons/deleteRow.png" alt="<spring:message code='removeThisOption'/>" title="<spring:message code='removeThisOption'/>"/></a></td>
		</tr>
